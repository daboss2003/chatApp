  ...
  if (error) {
    ...
    return (
      <div className={` w-full relative ${!globalView.showNav ? 'h-[100vh]' : 'h-full'}`}>
         <header className='md:bg-light dark:bg-dark h-[10%] max-h-[10%] w-full flex items-center justify-between px-3 bg-colorTeal md:text-inherit text-light'>
            ...
            <figure className='w-[45px] rounded-full '><img src={sender.imageUrl} alt="" className='w-[30px] h-[30px] rounded-full' /></figure>
                <div>
                 <p className='tracking-wide font-bold '>{sender.username}</p>
                  <p className=' text-xs tracking-wide'>{sender?.active ? 'online' : 'offline'}</p>
          ...
          <div className='lg:static absolute top-20 bg-inherit right-0'>
            <Button type={'call'} onClick={handleStarred}>
              {
                currentMessage.starred ? <AiFillStar size={20} /> : <AiOutlineStar size={20} />
              }
            </Button>
            <Button type={'call'} onClick={handleArchived}>
              {
                currentMessage.archived ? <MdArchive size={20} /> : <MdOutlineArchive size={20} />
              }
            </Button>
          </div>
          {window.innerWidth >= 768 &&
            <div className='pr-6 flex items-center justify-center'>
                  <Button type={'call'}><SlCamrecorder size={20} /></Button>
                  <Button type={'call'}><LuPhone size={20} /></Button>
            </div>
          }
          {window.innerWidth < 768 && 
        ...
        <div className='max-h-[80%] h-[80%] overflow-x-hidden overflow-y-auto px-6 py-4 bg-[url(img/bg.jpg)] bg-no-repeat bg-cover bg-center dark:bg-[url(img/bg-dark.webp)]'>
          ...
          <form className='md:h-[10%] flex items-center bg-light dark:bg-dark px-4 gap-3 md:rounded-none rounded-3xl md:w-full w-[86%] shadow-md h-[9%] md:ml-0 ml-1' onSubmit={(e) => e.preventDefault()}>
              <div className='flex items-center gap-3 overflow-y-auto'>
               <Button onClick={handleShowEmoji}><GrEmoji size={20} /></Button>
              ...
              <div className='flex-1 bg-light dark:bg-dark flex items-center gap-2 '>
                ...
                ref={inputRef}
                 ...
                 ></textarea>
            <button className={`py-1 px-3 hover:bg-blueColor hover:text-light rounded-lg duration-300 border border-blueColor dark:text-light items-center justify-center ${input === '' ? 'hidden' : 'flex'}`} onClick={handleTextMessage}><IoMdSend size={20} /></button>
            <Button onClick={() => setMediaType('image')}><FaCamera size={20} /></Button>
            <Button onClick={() => setMediaType('video')}><SlCamrecorder size={20} /></Button>
          </div>
          {window.innerWidth < 768 &&  <div className='absolute right-0 max-w-[15%] flex justify-center items-center w-[14%]'><button className=' rounded-full bg-blueColor text-light p-4 shadow-md' onClick={handleStartRecording}><FiMic size={20} /></button></div>}
               ...
               <div className='flex items-center gap-3 lg:flex-row flex-col bg-inherit relative'>
                {mediaType ?
                   <div className='absolute md:-top-[300px] bottom-[50%] w-[70vw] md:h-[55vh] md:-left-[350px] -translate-x-[50%] -translate-y-[50%] bg-light rounded-md p-2 pt-0 dark:bg-darkLight md:min-w-[80%] md:bottom-0 md:max-h-[55vh] md:w-[40vw] webcam shadow-md'>
                    <Button onClick={handleCloseMediaType}><RxCross2 size={20} /></Button>
                    <Webcam
                ...
                {audioRecorder ?
                  <div className='absolute top-[65%] md:-top-[100px] md:-left-[200px] p-2 md:-translate-x-[50%] -translate-y-[65%] rounded-lg flex items-center justify-between bg-light dark:bg-darkLight max-w-[98vw] -translate-x-[35%] shadow-md'>
                    <Button onClick={handleCloseRecorder}><RxCross2 size={20} /></Button>
                <div className='bg-light md:p-2 p-1 rounded-lg flex m-2 items-center justify-between md:gap-4 gap-2 dark:bg-dark'><Button onClick={handleRecordingMotion}>{audioRecording ? <HiPause size={22} /> : <FaPlay size={20} /> }</Button> <progress value={audioLevel} className='h-2 rounded-lg' max={10000}></progress> <p className=''>{ time}</p></div>
                    <button className={`py-1 px-3 hover:bg-blueColor hover:text-light rounded-lg duration-300 border border-blueColor dark:text-light items-center justify-center`} onClick={handleUploadAudio}><IoMdSend size={20} /></button>
                  </div> : ''
                }
                
                {window.innerWidth >= 768 && <Button onClick={handleStartRecording}><FiMic size={20} /></Button>}
